	...
	for searchList.Len() != 0 {
		...
		for {
			...
			if labelLen == 0 {
				...
				fn(strings.ToLower(domainName.String()))
				break
			}
			...
			for i := 0; i < labelLen; i++ {
				...
				if !isLetter(b) && !isDigit(b) && b != '-' {
					return fmt.Errorf("invalid character (%b) for a domain name's label in an NDP Search List option: %w", b, ErrNDPOptMalformedBody)
				}

				if i == 0 && !isLetter(b) {
					return fmt.Errorf("first character of a domain name's label in an NDP Search List option must be a letter, got %c: %w", b, ErrNDPOptMalformedBody)
				}
