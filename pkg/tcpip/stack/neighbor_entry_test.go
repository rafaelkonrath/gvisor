...
// | Stale      | Stale      | Probe w/ different address                 | Update address  | StateChange |
...
func (d *testNUDDispatcher) OnNeighborStateChange(nicID tcpip.NICID, addr tcpip.Address, linkAddr tcpip.LinkAddress, state NeighborState, updatedAt time.Time) {
	d.queueEvent(testEntryEventInfo{
...
func TestEntryReachableToReachableWhenConfirmationWithRouterFlag(t *testing.T) {
	t.Parallel()
	c := DefaultNUDConfigurations()
	c.RetransmitTimer = time.Microsecond
