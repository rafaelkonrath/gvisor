...
// dispatchStateChangeEventLocked sends an event to notify listening users that
// as entry has changed its state in the NUD state machine as defined by
// RFC 4861 Section 7.3.2.
func (e *neighborEntry) dispatchStateChangeEventLocked(nextState NeighborState) {
	if e.nic.stack.nudDisp == nil {
		return
	}
	e.nic.stack.nudDisp.OnNeighborStateChange(e.nic.id, e.mu.neigh.Addr, e.mu.neigh.LinkAddr, nextState, time.Now())
}
...
func (e *neighborEntry) setStateLocked(next NeighborState) {
	...
	case Reachable:
		e.mu.timer = tcpip.MakeCancellableTimer(&e.mu, func() {
			e.dispatchStateChangeEventLocked(Stale)
	...
	case Delay:
		...
		e.mu.retryCounter = 0
		e.mu.timer = tcpip.MakeCancellableTimer(&e.mu, func() {
			e.dispatchStateChangeEventLocked(Probe)
...
func (e *neighborEntry) handlePacketQueuedLocked(linkRes LinkAddressResolver) {
	...
	case Stale:
		e.dispatchStateChangeEventLocked(Delay)
...
func (e *neighborEntry) handleConfirmationLocked(linkAddr tcpip.LinkAddress, flags ReachabilityConfirmationFlags) {
	...
	case Incomplete:
		...
		if flags.Solicited {
			e.dispatchStateChangeEventLocked(Reachable)
